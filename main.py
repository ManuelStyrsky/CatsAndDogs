...
def get_class_dataset(dataset, label, class_name):
    ...
    images = files.map(lambda x: (x, label), 8)
    return images
...
def get_dataset(dataset='train', shuffle=True, batch_size=250, buffer_size=20000, repeat=True, prefetch=500):
    ...
    data = data.map(prepare_image_fn, 5)
...
def cnn_model_fn(features, labels, mode):
    ...
    pool1 = tf.layers.max_pooling2d(inputs=norm1, pool_size=[5, 5], strides=4)
    ...
    pool2 = tf.layers.max_pooling2d(inputs=norm2, pool_size=[5, 5], strides=4) #4,4
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
        optimizer = tf.train.GradientDescentOptimizer(learning_rate=0.002) #change
...
def main(argv):
    ...
    classifier = tf.estimator.Estimator(
        model_fn=cnn_model_fn, model_dir='model/model_11')
